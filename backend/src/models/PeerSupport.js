...
class PeerSupport {
  ...
  static async joinGroup(userId, groupId, anonymousNickname) {
    // 2026-06-16: membership_id is generated by the DB via DEFAULT
    // uuid_generate_v4(); previously we passed a uuidv4() into a
    // non-existent `member_id` column, which raised "column does not
    // exist" at runtime. Drop the column from the INSERT entirely.
    ...
    const groupQuery = `
      SELECT g.max_members, COUNT(gm.membership_id) as current_members
      FROM peer_support_groups g
      LEFT JOIN group_members gm ON g.group_id = gm.group_id AND gm.is_active = true
      WHERE g.group_id = $1
      GROUP BY g.group_id
    ...
    // Add member
    const query = `
      INSERT INTO group_members (group_id, user_id, anonymous_nickname)
      VALUES ($1, $2, $3)
      RETURNING *
    `;

    const result = await db.query(query, [groupId, userId, anonymousNickname]);
    return result.rows[0];
  }
  ...
  static async getUserGroups(userId) {
    const query = `
      SELECT
        ...
        gm.is_moderator,
        gm.joined_at,
        COUNT(DISTINCT gm2.membership_id) as member_count
